...
begin
  ...
  TWX_ZeroMemory(Result, SizeOf(TDataHeader));
...
begin
  ...
  begin
    ...
    if Bad then
    begin
      TWX_SetForegroundWindow(Application.Handle);
...
begin
  TWX_ZeroMemory(@Que[0], (DBHeader.Sectors + 1) * SizeOf(Word));
  TWX_ZeroMemory(@ReverseArray[0], (DBHeader.Sectors + 1) * SizeOf(Word));
end;
...
begin
  Result := TList.Create;
  TWX_CopyMemory(@CheckedArray[1], @VoidArray[1], DBHeader.Sectors);
...
begin
  TWX_ZeroMemory(@Sector, SizeOf(Sector));
...
begin
  TWX_ZeroMemory(@Planet, SizeOf(Planet));
end;

procedure TModDatabase.NULLTrader(var Trader : TTrader);
begin
  TWX_ZeroMemory(@Trader, SizeOf(Trader));
end;

procedure TModDatabase.NULLShip(var Ship : TShip);
begin
  TWX_ZeroMemory(@Ship, SizeOf(Ship));
end;
...
begin
  ...
  begin
    ...
    if not (CompareMem(DataIndex, Data, Size)) then
      ...
      TWX_CopyMemory(DataIndex, Data, Size);
      Seek(DataFile, Index);
