...
begin
  ...
  LabelScriptName := Params[4].Value;

  if (LabelScriptName <> '') then
    TScript(Script).Cmp.ExtendLabelName(LabelScriptName, TScript(Script).ExecScript);

  TScript(Script).AddMenu(TWXMenu.AddCustomMenu(UpperCase(Params[0].Value),
    UpperCase(Params[1].Value), Params[2].Value, LabelScriptName, Params[5].Value,
...
begin
  // CMD: getAllCourses <2-DimensionArrayScriptName> <StartingSector>
...
begin
  // CMD: getMenuValue <menuScriptName> var
...
var
  ...
  CourseList : TList;
begin
  // CMD: getNearestWarps <ArrayScriptName> <StartingSector>
  ConvertToNumber(Params[1].Value, StartSect);
  CheckSector(StartSect);
...
var
  ...
  Hdr : ShortHdr;
begin
  // CMD: getScriptVersion <fileScriptName> var
  ...
  begin
    ...
    if (Hdr.ProgramScriptName <> 'TWX SCRIPT') then begin
      CloseFile(F);
...
begin
  ...
  VarScriptName := TVarParam(Params[1]).Name;
  Result := caNone;

  with (Script as TScript) do
  begin
    if (S.Explored = etNo) then
      SetVariable(VarScriptName + '.EXPLORED', 'NO', '')
    else if (S.Explored = etCalc) then
      SetVariable(VarScriptName + '.EXPLORED', 'CALC', '')
    else if (S.Explored = etDensity) then
      SetVariable(VarScriptName + '.EXPLORED', 'DENSITY', '')
    else if (S.Explored = etHolo) then
      SetVariable(VarScriptName + '.EXPLORED', 'YES', '');

    SetVariable(VarScriptName + '.INDEX', IntToStr(Index), '');
    SetVariable(VarScriptName + '.BEACON', S.Beacon, '');
    SetVariable(VarScriptName + '.CONSTELLATION', S.Constellation, '');
    SetVariable(VarScriptName + '.ARMIDMINES.QUANTITY', IntToStr(S.Mines_Armid.Quantity), '');
    SetVariable(VarScriptName + '.LIMPETMINES.QUANTITY', IntToStr(S.Mines_Limpet.Quantity), '');
    SetVariable(VarScriptName + '.ARMIDMINES.OWNER', S.Mines_Armid.Owner, '');
    SetVariable(VarScriptName + '.LIMPETMINES.OWNER', S.Mines_Limpet.Owner, '');
    SetVariable(VarScriptName + '.FIGS.QUANTITY', IntToStr(S.Figs.Quantity), '');
    SetVariable(VarScriptName + '.FIGS.OWNER', S.Figs.Owner, '');
    SetVariable(VarScriptName + '.WARPS', IntToStr(S.Warps), '');
    SetVariable(VarScriptName + '.DENSITY', IntToStr(S.Density), '');
    SetVariable(VarScriptName + '.NAVHAZ', IntToStr(S.NavHaz), '');

    for I := 1 to 6 do
      SetVariable(VarScriptName + '.WARP', IntToStr(S.Warp[I]), IntToStr(I));

    SetVariable(VarScriptName + '.UPDATED', DateToStr(S.Update) + ' ' + TimeToStr(S.Update), '');
    SetVariable(VarScriptName + '.PORT.NAME', S.SPort.Name, '');

    if (S.Figs.FigType = ftToll) then
      SetVariable(VarScriptName + '.FIGS.TYPE', 'TOLL', '')
    else if (S.Figs.FigType = ftDefensive) then
      SetVariable(VarScriptName + '.FIGS.TYPE', 'DEFENSIVE', '')
    else
      SetVariable(VarScriptName + '.FIGS.TYPE', 'OFFENSIVE', '');

    // MB - Added mispelled paramter for backwards compatability
    if (S.Anomaly) then begin
      SetVariable(VarScriptName + '.ANOMALY', 'YES', '');
      SetVariable(VarScriptName + '.ANOMOLY', 'YES', '')
    end
    else begin
      SetVariable(VarScriptName + '.ANOMALY', 'NO', '');
      SetVariable(VarScriptName + '.ANOMOLY', 'NO', '');
    end;

    if (S.SPort.Name = '') then
    begin
      SetVariable(VarScriptName + '.PORT.CLASS', '0', '');
      SetVariable(VarScriptName + '.PORT.EXISTS', '0', '');
    end
    else
    begin
      SetVariable(VarScriptName + '.PORT.CLASS', IntToStr(S.SPort.ClassIndex), '');
      SetVariable(VarScriptName + '.PORT.EXISTS', '1', '');
      SetVariable(VarScriptName + '.PORT.BUILDTIME', IntToStr(S.SPort.BuildTime), '');
      SetVariable(VarScriptName + '.PORT.PERC_ORE', IntToStr(S.SPort.ProductPercent[ptFuelOre]), '');
      SetVariable(VarScriptName + '.PORT.PERC_ORG', IntToStr(S.SPort.ProductPercent[ptOrganics]), '');
      SetVariable(VarScriptName + '.PORT.PERC_EQUIP', IntToStr(S.SPort.ProductPercent[ptEquipment]), '');
      SetVariable(VarScriptName + '.PORT.ORE', IntToStr(S.SPort.ProductAmount[ptFuelOre]), '');
      SetVariable(VarScriptName + '.PORT.ORG', IntToStr(S.SPort.ProductAmount[ptOrganics]), '');
      SetVariable(VarScriptName + '.PORT.EQUIP', IntToStr(S.SPort.ProductAmount[ptEquipment]), '');
      SetVariable(VarScriptName + '.PORT.UPDATED', DateToStr(S.SPort.Update) + ' ' + TimeToStr(S.SPort.Update), '');

      if (S.SPort.BuyProduct[ptFuelOre]) then
        SetVariable(VarScriptName + '.PORT.BUY_ORE', 'YES', '')
      else
        SetVariable(VarScriptName + '.PORT.BUY_ORE', 'NO', '');

      if (S.SPort.BuyProduct[ptOrganics]) then
        SetVariable(VarScriptName + '.PORT.BUY_ORG', 'YES', '')
      else
        SetVariable(VarScriptName + '.PORT.BUY_ORG', 'NO', '');

      if (S.SPort.BuyProduct[ptEquipment]) then
        SetVariable(VarScriptName + '.PORT.BUY_EQUIP', 'YES', '')
      else
        SetVariable(VarScriptName + '.PORT.BUY_EQUIP', 'NO', '');
    end;

    // set planet variables
    Items := TWXDatabase.GetSectorItems(itPlanet, S);
    SetVariable(VarScriptName + '.PLANETS', IntToStr(Items.Count), '');
    I := 0;

    while (Items.Count > 0) do
    begin
      Inc(I);
      SetVariable(VarScriptName + '.PLANET', TPlanet(Items[0]^).Name, IntToStr(I));
      FreeMem(Items[0]);
      Items.Delete(0);
    end;
    ...
    SetVariable(VarScriptName + '.TRADERS', IntToStr(Items.Count), '');
    I := 0;

    while (Items.Count > 0) do
    begin
      Inc(I);
      SetVariable(VarScriptName + '.TRADER.NAME', TTrader(Items[0]^).Name, IntToStr(I));
      SetVariable(VarScriptName + '.TRADER.SHIP', TTrader(Items[0]^).ShipType, IntToStr(I));
      SetVariable(VarScriptName + '.TRADER.SHIPNAME', TTrader(Items[0]^).ShipName, IntToStr(I));
      SetVariable(VarScriptName + '.TRADER.FIGS', IntToStr(TTrader(Items[0]^).Figs), IntToStr(I));
    ...
    SetVariable(VarScriptName + '.SHIPS', IntToStr(Items.Count), '');
    I := 0;

    while (Items.Count > 0) do
    begin
      Inc(I);
      SetVariable(VarScriptName + '.SHIP.NAME', TShip(Items[0]^).Name, IntToStr(I));
      SetVariable(VarScriptName + '.SHIP.SHIP', TShip(Items[0]^).ShipType, IntToStr(I));
      SetVariable(VarScriptName + '.SHIP.OWNER', TShip(Items[0]^).Owner, IntToStr(I));
      SetVariable(VarScriptName + '.SHIP.FIGS', IntToStr(TTrader(Items[0]^).Figs), IntToStr(I));
      FreeMem(Items.Items[0]);
      Items.Delete(0);
    end;
    ...
    begin
      ...
      begin
        Inc(I);
        SetVariable(VarScriptName + '.BACKDOOR', IntToStr(Word(Items[0]^)), IntToStr(I));
        FreeMem(Items[0]);
        Items.Delete(0);
      end;
...
var
  Index: Integer;
begin
  // CMD: getSectorParameter <sectorIndex> <parameterScriptName> var
...
begin
  // CMD: killWindow <windowScriptName>
...
var
  ...
  Scripts : TStringList;
begin
  // CMD: listActiveScripts <ArrayScriptName>
...
var
  Avoids : TList;
begin
  // CMD: listAvoids <ArrayScriptName>
...
begin
  // CMD: listSectorParameters <SectorIndex> <ArrayScriptName>
...
var
  I : Integer;
  VarScriptNames, Values : TStringList;
begin
  // MB - Create List of Globals
  VarScriptNames:= TStringList.Create;
  Values:= TStringList.Create;

  try
    for I := 0 to TWXGlobalVars.Count - 1 do
    begin
      VarScriptNames.add(TGlobalVarItem(TWXGlobalVars[I]).Name);
      Values.add(TGlobalVarItem(TWXGlobalVars[I]).Value);
    end;

    Params[0].Value := IntToStr(VarScriptNames.Count);
    TVarParam(Params[0]).SetArrayFromStrings(VarScriptNames);
...
//  VarScriptNames:= TStringList.Create;
//  try
//    VarScriptName := StringReplace(TVarParam(Params[0]).Name, '$', '', [rfReplaceAll, rfIgnoreCase]);
//    Split(VarScriptName, VarScriptNames, '~');
//    VarScriptName := VarScriptNames[VarScriptNames.count - 1];
//  finally
//    VarScriptNames.free;
//  end;
...
begin
  ...
  LastFileTime := TWX_GetFileTime(Params[0].Value);
  if (LastFileTime <> 0) then
  begin
    // If the file name or timestamp has changed, update the constants
    if (Params[0].Value <> LastReadFilename) or (DateTimeToFileDate(LastFileTime) <> LastReadModTime) then
    begin
      LastReadFilename := Params[0].Value;
      LastReadModTime := DateTimeToFileDate(LastFileTime);
...
begin
  ...
  //Find := TGlobalVarItem.Create(VarScriptName, '0');
...
begin
  // CMD: setMenuHelp <menuScriptName> <helpText>
...
begin
  // CMD: setMenuValue <menuScriptName> <value>
...
begin
  // CMD: setMenuOptions <menuScriptName> <Q> <?> <+>
...
begin
  // CMD: setProgVar <varScriptName> <value>
...
var
  Index: Integer;
begin
  // CMD: setSectorParameter <sectorIndex> <parameterScriptName> <value>
...
begin
  // CMD: setWindowContents <windowScriptName> <value>
...
var
  ...
  ScriptName : string;
begin
  // CMD: stop <scriptScriptName>
  ScriptName := UpperCase(ShortFilename(StripFileExtension(Params[0].Value)));
  Result := caNone;

  for I := 0 to TWXInterpreter.Count - 1 do
    if (UpperCase(ShortFilename(StripFileExtension(TWXInterpreter.Scripts[I].Cmp.ScriptFile))) = ScriptName) then
    begin
...
var
  ...
  SizeY  : Integer;
begin
  // CMD: window <windowScriptName> <sizeX> <sizeY> <title> [<ontop>]
  ...
  try
    ...
    begin
      ...
      begin
        ...
        ScriptName  := IniFile.ReadString(Section, 'ScriptName', '');
        ScriptFile  := IniFile.ReadString(Section, 'Script', '');
        ScriptNameVar  := IniFile.ReadString(Section, 'ScriptNameVar', '');

         BotScriptName := '~f{~c~f}';
        if Length(ScriptNameVar) > 0 then
          if Pos('file:', LowerCase(ScriptNameVar)) = 0 then
            BotScriptName := '~f{~c' + INI.ReadString('Variables', ScriptNameVar, '0') + '~f}'
          else
          begin
            FileScriptName := StringReplace(ScriptNameVar, 'FILE:', '', [rfReplaceAll, rfIgnoreCase]);
            FileScriptName := StringReplace(FileScriptName, '{GAME}', StringReplace(StripFileExtension(TWXDatabase.DatabaseName),'data\', '', [rfReplaceAll, rfIgnoreCase]), [rfReplaceAll, rfIgnoreCase]);
            if (FileExists(TWXGUI.ProgramDir + '\' + FileScriptName)) then
            begin
              try
                fileData.Clear();
                fileData.LoadFromFile(TWXGUI.ProgramDir + '\' + FileScriptName);
                BotScriptName := '~f{~c' + fileData[0] + '~f}';
              finally
              end;
            end;
          end;

          if BotScriptName = '~f{~c0~f}' then
            BotScriptName := '~f{~c~f}';
        ...
        begin
          if Pos(LowerCase(ScriptFile), LowerCase(TWXInterpreter.ActiveBotScript)) > 0 then
            //BotList.add(Format('~D>~C%-8s ~G%s', [Alias, BotScriptName]))
            BotList.add(Format('%-14s ~G%-6s ~F%s ~B<ACTIVE>', [BotScriptName, Alias, ScriptName]))
          else
            //BotList.add(Format('~C %-8s ~G%s', [Alias, BotScriptName]));
            BotList.add(Format('%-14s ~G%-6s ~F%s', [BotScriptName, Alias, ScriptName]))
          end;
        end;
      end;
  finally
...
begin
  ...
  TWX_ShellExecute(TWXGUI.ProgramDir + '\twxp.exe ' + P);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if TWX_GetCurrentProcessId() <> ProcessID then
        begin
          Handle := TWX_OpenProcess(ProcessID);
          if Handle > 0 then
            TWX_TerminateProcess(Handle, ProcessID);
...
procedure ClearScriptData(ScriptName: string);
var
  searchFile : TSearchRec;
begin
    TWXServer.ClientMessage('Clearing script data files.');
    try
      if findfirst(TWXGUI.ProgramDir + '\*_' + ScriptName + '*.*', faAnyFile, searchFile) = 0 then
      repeat
        DeleteFile(pchar(searchFile.Name));
      until FindNext(searchFile) <> 0;
      SysUtils.FindClose(searchFile);

      if findfirst(TWXGUI.ProgramDir + '\data\' + ScriptName + '\*.*', faAnyFile, searchFile) = 0 then
      repeat
        DeleteFile(pchar(TWXGUI.ProgramDir + '\data\' + ScriptName + '\' + searchFile.Name));
      until FindNext(searchFile) <> 0;
      SysUtils.FindClose(searchFile);

      if findfirst(TWXGUI.ProgramDir + '\scripts\Mombot\Games\' + ScriptName + '\*.*', faAnyFile, searchFile) = 0 then
      repeat
        DeleteFile(pchar(TWXGUI.ProgramDir + '\scripts\Mombot\Games\' + ScriptName + '\' + searchFile.Name));
      until FindNext(searchFile) <> 0;
      SysUtils.FindClose(searchFile);
      RemoveDir(TWXGUI.ProgramDir + '\scripts\Mombot\Games\' + ScriptName);
    finally
      SysUtils.FindClose(searchFile);
    end;
end;
...
begin
  ...
  TWX_CopyFile(TWXGUI.ProgramDir + '\data\' + Source + '.xdb',
               TWXGUI.ProgramDir + '\data\' + Dest + '.xdb');
...
var
  ...
  Head     : PDataHeader;
begin
  // CMD: CreateDatabase <ScriptName> <Sectors> [Address] [ServerPort] [LisenPort]
  ...
  Head := GetBlankHeader;
//DBHeader.ProgramScriptName <> 'TWX DATABASE'
...
begin
  ...
  HFileRes := TWX_CreateFile('data\' + Database + '.xdb', 0, 0);
  if HFileRes = Cardinal(-1) then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      TWX_QueryPerformanceFrequency(Frequency);
      TWX_QueryPerformanceCounter(EndTime);
...
begin
  ...
  begin
    FileScriptName := TScript(Script).ProgramDir + '\scripts\' +
                TWXInterpreter.ActiveBotDir + '\help\' + Params[1].Value + '.txt';

    if (not FileExists(FileScriptName)) or (PARM1 = '?') then
    begin
      S := Params[1].Value + ' - ' + Params[0].Value;
      TWXExtractor.StripANSI(S);

      try
        FS := TFileStream.Create(FileScriptName, fmCreate or fmOpenReadWrite);
        try
          FS.Write(S, SizeOf(S));
        finally
          FS.Free;
...
function SCGameScriptName(Indexes : TStringArray) : string;
begin
  Result := StripFileExtension(ShortFileName(TWXDatabase.DatabaseName));
end;

function SCLicenseScriptName(Indexes : TStringArray) : string;
begin
  Result := 'User'
end;
...
function SCPort_ScriptName(Indexes : TStringArray) : string;
var
  SectIndex : Integer;
begin
  if (Length(Indexes) < 1) then
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if Length(ScriptNameVar) > 0 then
          if Pos('file:', LowerCase(ScriptNameVar)) = 0 then
            BotScriptName := '~3{~4' + INI.ReadString('Variables', ScriptNameVar, '') + '~3}'
          else
          begin
            FileScriptName := StringReplace(ScriptNameVar, 'FILE:', '', [rfReplaceAll, rfIgnoreCase]);
            FileScriptName := StringReplace(FileScriptName, '{GAME}', StringReplace(StripFileExtension(TWXDatabase.DatabaseName),'data\', '', [rfReplaceAll, rfIgnoreCase]), [rfReplaceAll, rfIgnoreCase]);
            ...
            begin
              try
                fileData := TStringList.Create;
                fileData.LoadFromFile(TWXGUI.ProgramDir + '\' + FileScriptName);
                BotScriptName := '~3{~4' + fileData[0] + '~3}';
              finally
                fileData.Free;
              end;
            end;
...
//        if BotScriptName = '~c{~f0~c}' then
//          BotScriptName := '~c{}';

        if FileExists (TWXGUI.ProgramDir + '\scripts\' + ScriptFile) then
        begin
          BotList := BotList + Format('  %-14s ~1%-6s ~2%s', [BotScriptName, Alias, ScriptName]);
          if Pos(LowerCase(ScriptFile), LowerCase(TWXInterpreter.ActiveBotScript)) > 0 then
...
begin
  ...
  begin
    ...
    AddSysConstant('GAMENAME', SCGameScriptName);
    AddSysConstant('LICENSENAME', SCLicenseScriptName);
    ...
    AddSysConstant('PORT.NAME', SCPort_ScriptName);
