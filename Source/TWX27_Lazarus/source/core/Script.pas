...
type
  ...
  private
    ...
    function CreateTrigger(TriggerName, LabelName, Value : string) : PTrigger;
    function GetProgramDir: string;
  ...
  public
    constructor Create(ModOwner : TModInterpreter); reintroduce;
    ...
    function EventActive(EventName : string) : Boolean;
    procedure SetAutoTrigger(TriggerName, Value, Response : String; LifeCycle : Integer);
    procedure SetTextLineTrigger(TriggerName, LabelName, Value : string);
    procedure SetTextOutTrigger(TriggerName, LabelName, Value : string);
    procedure SetTextTrigger(TriggerName, LabelName, Value : string);
    procedure SetEventTrigger(TriggerName, LabelName, Value, Param : string);
    procedure SetDelayTrigger(TriggerName, LabelName : string; Value : Integer);
    procedure KillTrigger(TriggerName : string);
...
begin
  ...
  GlobalScriptList := TStringList.Create;
  ...
  begin
    ...
    try
    ExtractStrings([','], [], PChar(ScriptName), GlobalScriptList);
    for script in GlobalScriptList do
    begin
...
constructor TScript.Create(ModOwner : TModInterpreter);
var
  TriggerType: TTriggerType;
begin
  inherited Create(ModOwner);
...
function TScript.TriggerExists(TriggerName : string) : Boolean;
var
  I: Integer;
  TriggerList: TList;
  TriggerType: TTriggerType;
...
function TScript.CreateTrigger(TriggerName, LabelName, Value : String) : PTrigger;
begin
  Cmp.ExtendName(TriggerName, ExecScriptID);
  Cmp.ExtendLabelName(LabelName, ExecScriptID);

  if (TriggerExists(TriggerName)) then
    raise EScriptError.Create('Trigger already exists: ''' + TriggerName + '''');

  Result := AllocMem(SizeOf(TTrigger));
  Result.Name := TriggerName;
...
procedure TScript.SetAutoTrigger(TriggerName, Value, Response : String; LifeCycle : Integer);
var
  Trigger : PTrigger;
begin
...
procedure TScript.SetTextLineTrigger(TriggerName, LabelName, Value : String);
begin
  FTriggers[ttTextLine].Add(CreateTrigger(TriggerName, LabelName, Value));
end;

procedure TScript.SetTextOutTrigger(TriggerName, LabelName, Value : String);
begin
  FTriggers[ttTextOut].Add(CreateTrigger(TriggerName, LabelName, Value));
end;

procedure TScript.SetTextTrigger(TriggerName, LabelName, Value : String);
begin
  FTriggers[ttText].Add(CreateTrigger(TriggerName, LabelName, Value));
end;

procedure TScript.SetEventTrigger(TriggerName, LabelName, Value, Param : string);
var
  Trigger : PTrigger;
begin
  Cmp.ExtendName(TriggerName, ExecScriptID);
...
procedure TScript.SetDelayTrigger(TriggerName, LabelName : string; Value : Integer);
var
  Trigger : PTrigger;
begin
  Cmp.ExtendName(TriggerName, ExecScriptID);
...
procedure TScript.KillTrigger(TriggerName : string);
var
  I: Integer;
  Found: Boolean;
  TriggerList: TList;
