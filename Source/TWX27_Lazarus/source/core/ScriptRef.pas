...
type
  ...
  public
    ...
    procedure AddCommand(CommandName : string; MinParams, MaxParams : Integer; onCmd : TScriptCmdHandler;
      ParamKinds : array of TParamKind; DefParamKind : TParamKind);
    procedure AddSysConstant(ConstantName : string; onRead : TScriptConstHandler);

    function FindCmd(CommandName : string) : Integer;
    function FindSysConst(ConstantName : string) : Integer;
...
begin
  ...
  begin
    ...
    begin
      FStrValue := FloatToStrF(FDecValue, ffFixed, 18, FSigDigits);
      FNumChanged := FALSE;
    end;
  end;
  Result := FStrValue;
...
function TCmdParam.GetDecValue: Extended;
var
  F : Extended;
  DecPos: Integer;
begin
...
procedure TScriptRef.AddCommand(CommandName : string; MinParams, MaxParams : Integer;
...
procedure TScriptRef.AddSysConstant(ConstantName : string; onRead : TScriptConstHandler);
var
  NewConst : TScriptSysConst;
begin
  // build a new system constant and add it to the list
  NewConst := TScriptSysConst.Create;
  NewConst.Name := ConstantName;
...
begin
  ...
  Result := TScriptCmd(CmdList.Items[Index]);
end;

function TScriptRef.GetScriptSysConst(Index : Integer) : TScriptSysConst;
begin
  // retrieve a system const from list
  Result := TScriptSysConst(SysConstList.Items[Index]);
end;

function TScriptRef.FindCmd(CommandName : string) : Integer;
var
  I : Integer;
begin
  Result := -1;
  CommandName := UpperCase(CommandName);

  if (CmdList.Count > 0) then
    for I := 0 to CmdList.Count - 1 do
      if (TScriptCmd(CmdList[I]).Name = CommandName) then
      begin
        Result := I;
        Exit;
      end;
...
function TScriptRef.FindSysConst(ConstantName : string) : Integer;
var
  I : Integer;
begin
  Result := -1;
  ...
  if (SysConstList.Count > 0) then
    for I := 0 to SysConstList.Count - 1 do
      if (TScriptCmd(SysConstList[I]).Name = ConstantName) then
      begin
        Result := I;
        Exit;
      end;
