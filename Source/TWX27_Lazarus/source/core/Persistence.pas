...
begin
  ...
  try
    ...
    begin
      ...
      {$IFDEF WINDOWS}OutputDebugString(PChar('Saving Module #' + IntToStr(i) ));{$ENDIF}

      // MB - Skipping mtMenu because it is throwing exceptions.
      if (i <> 3) then
      begin
        Module.GetStateValues(ModuleValues);
        {$IFDEF WINDOWS}OutputDebugString(PChar('Module size:' + IntToStr(ModuleValues.Size) ));{$ENDIF}
