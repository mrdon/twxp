...
type
  ...
  private
    ...
    procedure SetSocket;
    procedure OnFSocketError(Sender : TObject);
    procedure OnFSocketRead(Sender : TObject);
    procedure OnFSocketConnect(Sender : TObject);
...
begin
  if not Authenticate or (FSocket.Connected) then
    Exit;
...
procedure TModAuth.OnFSocketError(Sender : TObject);
begin
  FSocket.Disconnect;
...
procedure TModAuth.OnFSocketRead(Sender : TObject);
...
begin
  ...
  FAuthLine := FAuthLine + StringReplace(FSocket.ReceiveText, #10, '', [rfReplaceAll]);
  I := 1;
...
procedure TModAuth.OnFSocketConnect(Sender : TObject);
var
  AuthOp  : Char;
  IDKey1,
  IDKey2,
...
begin
  ...
  // Simplified hardware ID for open source version
  IDKey1 := 30;
  IDKey2 := 20;
...
begin
  // Simplified checksum for open source version
  FChecksum := 12345;
end;
