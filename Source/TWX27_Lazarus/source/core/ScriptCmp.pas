...
type
  ...
  protected
    ...
    procedure BuildLabel(const LabelName : string; Location : Integer);
  ...
  public
    ...
    procedure ExtendName(var ScriptName : string; ScriptID : Integer);
    procedure ExtendLabelName(var LabelName : string; ScriptID : Integer);
...
procedure TScriptCmp.BuildLabel(const LabelName : string; Location : Integer);
var
  I        : Integer;
  CurLabel,
...
procedure TScriptCmp.ExtendName(var ScriptName : string; ScriptID : Integer);
begin
  if (Pos('~', ScriptName) = 0) then
  begin
    if (ScriptID > 0) then
      ScriptName := IncludeScriptList[ScriptID] + '~' + ScriptName;
  end
  else
  begin
    if (ScriptName <> '') then
      ...
      begin
        ...
        ScriptName := Copy(ScriptName, 2, Length(ScriptName));
      end;
  end;
end;

procedure TScriptCmp.ExtendLabelName(var LabelName : string; ScriptID : Integer);
begin
  if (Pos('~', LabelName) = 0) and (ScriptID > 0) then
    LabelName := ':' + IncludeScriptList[ScriptID] + '~' + Copy(LabelName, 2, Length(LabelName));
end;
