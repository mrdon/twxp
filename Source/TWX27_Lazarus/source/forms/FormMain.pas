...
uses
  ...
  Messages, ShellApi, WinInet,{$ENDIF}
  SysUtils,
  DateUtils,
  Classes,
  Graphics,
  ...
  Dialogs,
  Menus,
  {$IFDEF WINDOWS}ScktComp,{$ENDIF}
  ExtCtrls,
  StdCtrls,
  ComCtrls,
  {$IFDEF WINDOWS}CommCtrl,{$ENDIF}
  {$IFNDEF WINDOWS}LCLIntf,{$ENDIF}
  //TrayIcon,
  ImgList,
  FileCtrl,
  //Encryptor,
...
function EnumRCDataProc(hModule: HMODULE; lpszType, lpszName: PChar; lParam:
    NativeInt): {$IFDEF WINDOWS}BOOL{$ELSE}Boolean{$ENDIF}; stdcall;
begin
...
var
  ...
  BytesRead : dWord;
  {$ELSE}
  HttpClient: String; // Simplified for cross-platform
  {$ENDIF}
  Result,
  Update    : string;
begin
  ...
  {$ELSE}
  // Cross-platform HTTP - simplified to avoid external dependencies
  // In production, would use fphttpclient or synapse HTTP client
  Result := ''; // Disabled for now - can be implemented with synapse
  {$ENDIF}
  //else
      ...
      begin
        ...
        begin
          ...
          'Would you like to download it now?',
			    'Checking for Updates', {$IFDEF WINDOWS}MB_YESNO{$ELSE}0{$ENDIF}) = {$IFDEF WINDOWS}IDYES{$ELSE}1{$ENDIF} then
          {$IFDEF WINDOWS}
  ShellExecute(Handle,'open','https://github.com/TW2002/TWXP/wiki',nil,nil, SW_SHOWNORMAL);
  {$ELSE}
  OpenURL('https://github.com/TW2002/TWXP/wiki');
  {$ENDIF}
        end;
      end
      else
      begin
        ...
        begin
          ...
          Application.MessageBox('You are running the latest version of TWX Proxy.',
			    'Checking for Updates', {$IFDEF WINDOWS}MB_OK{$ELSE}0{$ENDIF})
        end;
      end;
  finally
    IniFile.Free;
