...
var
  ...
  Head     : PDataHeader;
  DoCreate : Boolean;
  HFileRes : {$IFDEF WINDOWS}HFILE{$ELSE}THandle{$ENDIF};
...
begin
  ...
  {$ELSE}
  // Cross-platform icon loading - use default icon
  TrayImage.Canvas.Pen.Color := clBtnFace;
  TrayImage.Canvas.Brush.Color := clBtnFace;
  TrayImage.Canvas.FillRect(Rect(0,0,32,32));
  // Draw a simple default icon or load from resources
  {$ENDIF}
end;
...
{$ELSE}
  If False Then // Cross-platform fallback - disable icon picker on non-Windows
{$ENDIF}
  begin
...
procedure TfrmSetup.ClearScriptData(ScriptName: string);
var
  Result : Integer;
...
begin
    ...
    try
      if findfirst(FProgramDir + '\*_' + ScriptName + '*.*', faAnyFile, searchFile) = 0 then
      repeat
        DeleteFile(searchFile.Name);
      until FindNext(searchFile) <> 0;
      FindClose(searchFile);

      if findfirst(FProgramDir + '\data\' + ScriptName + '\*.*', faAnyFile, searchFile) = 0 then
      repeat
        DeleteFile(FProgramDir + '\data\' + ScriptName + '\' + searchFile.Name);
      until FindNext(searchFile) <> 0;
      FindClose(searchFile);
      //RemoveDir(FProgramDir + '\data\' + ScriptName);

      if findfirst(FProgramDir + '\scripts\Mombot\Games\' + ScriptName + '\*.*', faAnyFile, searchFile) = 0 then
      repeat
        DeleteFile(FProgramDir + '\scripts\Mombot\Games\' + ScriptName + '\' + searchFile.Name);
      until FindNext(searchFile) <> 0;
      FindClose(searchFile);
      RemoveDir(FProgramDir + '\scripts\Mombot\Games\' + ScriptName);
    finally
      FindClose(searchFile);
    end;
end;
